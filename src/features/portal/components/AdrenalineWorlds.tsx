...
import { Calendar, Clock, Rocket, Droplet } from 'lucide-react';
import { MOCK_ATTRACTIONS } from '../data/mockAttractions';
import { Attraction } from '../types';

function renderAttractionCard(attr: Attraction) {
  ...
  return (
    <div key={attr.id} className={baseClasses} style={{ minHeight: attr.layout.rowSpan === 2 ? '500px' : '240px' }}>
      ...
      <div className="absolute inset-0 bg-gradient-to-t  z-20 from-black/90 via-black/40 to-transparent p-6 flex flex-col justify-end text-white">
        ...
        {attr.layout.customStyle === 'crimson' && (
          <div className="flex flex-col h-full justify-between">
            ...
            <div>
              <h3 className="text-4xl font-normal tracking-tight mb-3 uppercase">{attr.title}</h3>
        ...
        {attr.layout.customStyle === 'sky' && (
          <div className="flex flex-col h-full justify-center pl-4 lg:pl-10">
            <div className="text-[10px] tracking-[0.2em] uppercase mb-2 text-white/80 font-medium">{attr.category}</div>
            <h3 className="text-3xl font-medium tracking-wide uppercase mb-5">{attr.title}</h3>
        ...
        {attr.layout.customStyle === 'nebula' && (
          <div className="flex flex-col h-full justify-between">
            ...
            <div>
              <h3 className="text-lg font-medium tracking-wide uppercase mb-2 text-white/90 leading-tight">
        ...
        {attr.layout.customStyle === 'phoenix' && (
          <div className="flex flex-col h-full justify-between lg:justify-end">
            ...
            <div className="flex flex-col lg:flex-row lg:items-end justify-between items-start gap-4">
              <h3 className="text-3xl font-bold tracking-tight uppercase leading-none">{attr.title}</h3>
        ...
        {attr.layout.customStyle === 'midas' && (
          <div className="flex flex-col h-full justify-end pb-2">
            <h3 className="text-3xl font-normal tracking-tight mb-2 uppercase">{attr.title}</h3>
      ...
      <div className="max-w-7xl mx-auto pt-8">
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end mb-10 gap-x-12 gap-y-6">
          <div className="max-w-3xl">
            <h2 className="text-5xl md:text-6xl text-center font-black text-primary tracking-tight mb-4 antialiased">
              Our <span className="text-secondary italic font-bold">Games</span>
            </h2>
            {/* <div className="space-y-4 max-w-xl">
              <p className="text-[#68707a] text-[15px] leading-relaxed">
                Experience the pinnacle of engineering and excitement. From high-speed vertical drops to family-friendly adventures, our worlds are crafted for every heartbeat.
              </p>
              <p className="text-[#518ebc] text-sm leading-relaxed" dir="rtl">
                اختبر قمة الهندسة والإثارة. من السقوط العمودي عالي السرعة إلى المغامرات العائلية، عوالمنا مصممة لكل نبضة قلب.
              </p>
            </div> */}
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 md:grid-rows-3 gap-4">
          {MOCK_ATTRACTIONS.map(renderAttractionCard)}
        </div>
      </div>
    </section>
  );
